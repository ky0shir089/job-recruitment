...
const FiveC = () => {
  ...
  const { getFiveC, deleteFiveC } = useFiveC();
  ...
  const initialize = async () => {
    await getFiveC(currentPage, search).then((res) => {
      let { data } = res.data;
      setFiveC(data.data);
  ...
  const doSearch = async (e) => {
    if (e.keyCode == 13) {
      await getFiveC(1, search).then((res) => {
        let { data } = res.data;
        setFiveC(data.data);
        setLastPage(data.last_page);
      });
