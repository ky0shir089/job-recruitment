...
const Outlet = () => {
  ...
  const { getOutlet, deleteOutlet } = useOutlet();
  ...
  const initialize = async () => {
    await getOutlet(currentPage, search).then((res) => {
      let { data } = res.data;
      setOutlets(data);
  ...
  const doSearch = async (e) => {
    if (e.keyCode == 13) {
      await getOutlet(1, search).then((res) => {
        let { data } = res.data;
        setOutlets(data);
        setLastPage(data.last_page);
      });
                  ...
                  <Td>
                    <HStack spacing={2}>
                      <Button
                        ...
                        size="xs"
                        onClick={() =>
                          navigate(`${location.pathname}/${outlet.id}/edit`)
                        }
                      >
                        Edit
                      </Button>
