...
const Position = () => {
  ...
  const { getPosition, deletePosition } = usePosition();
  ...
  const initialize = async () => {
    await getPosition(currentPage, search).then((res) => {
      let { data } = res.data;
  ...
  const doSearch = async (e) => {
    if (e.keyCode == 13) {
      await getPosition(1, search).then((res) => {
        let { data } = res.data;
        setPositions(data.data);
        setLastPage(data.last_page);
      });
    ...
    <>
      ...
      <Box p={4}>
        <HStack justifyContent="space-between" mb={4}>
          <Heading>Setup Position</Heading>
                  ...
                  <Td>
                    <HStack spacing={2}>
                      <Button
                        ...
                        size="xs"
                        onClick={() =>
                          navigate(`${location.pathname}/${position.id}/edit`)
                        }
                      >
                        Edit
                      </Button>
